<?php
/**
 * management/edit_profile.php
 * ---------------------------------------------------------------------------
 * Management / Grievance Member — Edit Profile
 * Rajagiri College Grievance Redressal Portal
 *
 * Database Schema (grievance_db):
 *   users        : id, username, password, role, status
 *   cell_members : id, user_id, designation_id, department_id,
 *                  member_type, grievance_type_id, name, email,
 *                  address, mobile_number, whatsapp_number, profile_image
 *
 * Handles:
 *   • Pre-loading of member profile (users LEFT JOIN cell_members)
 *   • POST processing for cell_members + users tables
 *   • Profile picture upload (JPG/JPEG/PNG/WEBP, max 2 MB)
 *   • Secure password update with password_hash()
 *   • Redirect to management/profile.php on success
 * ---------------------------------------------------------------------------
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($contactNumber === '') {
        ...
        $formErrors[] = 'Please enter a valid mobile number (6–15 digits).';
    }
    ...
    if (empty($formErrors) && $conn !== null) {
        try {
            $chk = $conn->prepare("SELECT id FROM cell_members WHERE email = ? AND user_id != ? LIMIT 1");
            $chk->bind_param('si', $email, $userId);
            $chk->execute();
            if ($chk->get_result()->num_rows > 0) {
                $formErrors[] = 'This email is already registered to another member.';
            }
            $chk->close();
        } catch (Throwable $ex) {
    ...
    if (!empty($_FILES['profile_picture']['name']) && (int) $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileExt, $allowedExts, true)) {
            ...
            if (!is_dir($uploadDirAbs) || !is_writable($uploadDirAbs)) {
                ...
                $newFileName  = 'member_' . $userId . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $fileExt;
    ...
    if (empty($formErrors) && $conn !== null) {
        ...
        try {
            ...
            $oldProfileImage = '';
            $stmtOld = $conn->prepare("SELECT profile_image FROM cell_members WHERE user_id = ? LIMIT 1");
            if ($stmtOld) {
            ...
            // -------- 2) Update cell_members table (with address + whatsapp) --------
            if ($newProfilePictureRelative !== null) {
                // New image uploaded → save it
                $sqlMember = "UPDATE cell_members
                              SET name            = ?,
                                  address         = ?,
                                  email           = ?,
                                  mobile_number   = ?,
                ...
                if (!$stmt) {
                    throw new Exception('Prepare failed (cell_members update with image): ' . $conn->error);
                }
                ...
                $stmt = $conn->prepare($sqlMember);
                if (!$stmt) {
                    throw new Exception('Prepare failed (cell_members update): ' . $conn->error);
                }
            ...
            if (!$stmt->execute()) {
                throw new Exception('Failed to update member profile: ' . $stmt->error);
            }
            $stmt->close();
            ...
            error_log('[Management Edit Profile Save] ' . $ex->getMessage());
...
if ($conn !== null) {
    try {
        $sql = "SELECT  u.username,
                        ...
                        cm.whatsapp_number  AS whatsapp_number,
        ...
        if ($res && $res->num_rows > 0) {
            ...
            if ($_SERVER['REQUEST_METHOD'] !== 'POST' || $formData['contact_number'] === '') {
                $formData['contact_number'] = $row['mobile_number'] ?? '';
            }
        ...
        error_log('[Management Edit Profile Pre-load] ' . $ex->getMessage());
        if ($dbError === null) {
...
<head>
  ...
  <title>Edit Profile — Management | Rajagiri College Grievance Portal</title>
          ...
          <div class="relative" id="profile-dropdown-container">
            <button id="profile-dropdown-btn"
              ...
              <?php if ($existingPreviewUrl): ?>
                <img src="<?= e($existingPreviewUrl) ?>" alt="<?= e($formData['username'] ?: 'Member') ?>"
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-bold text-slate-800 truncate">
                      <?= e($formData['name'] ?: ($formData['username'] ?: 'Member')) ?>
                    </p>
                    <p class="text-xs text-slate-500 truncate">
                      <?= e($formData['email'] ?: 'management@rajagiri.edu') ?>
                    </p>
                  </div>
                </div>
              </div>
    ...
    <div id="logoutConfirmPanel"
      ...
      <div class="px-6 pt-6 pb-2 flex flex-col items-center text-center">
        ...
        <p class="text-sm text-slate-500 leading-relaxed">
          ...
          <span class="font-bold text-[#8B1E7E] break-words"><?= e($formData['name'] ?: ($formData['username'] ?: 'Member')) ?></span>.
