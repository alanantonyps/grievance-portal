...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $conn instanceof mysqli) {
    ...
    if ($action === 'reopen_grievance') {
        try {
            ...
            error_log('[Reopen Grievance] ' . $ex->getMessage());
            $_SESSION['flash_error'] = $ex->getMessage() ?: 'A system error occurred.';
        }
    ...
    if ($action === 'feedback_grievance') {
        try {
            ...
            error_log('[Feedback Grievance] ' . $ex->getMessage());
            $_SESSION['flash_error'] = $ex->getMessage() ?: 'A system error occurred.';
        }
...
if ($conn instanceof mysqli) {
    try {
        $stmt = $conn->prepare(
            "SELECT u.username, s.name, s.email, s.profile_image
             FROM users u
...
if (!empty($studentData['profile_image'])) {
    $rel = ltrim((string) $studentData['profile_image'], '/');
