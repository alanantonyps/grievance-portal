 ...
 *   parents : id, user_id, name, address, email, contact_number,
 *             whatsapp_number, profile_image, relation
 *
 * Handles:
 *   • Pre-loading of parent profile (users LEFT JOIN parents)
 *   • POST processing for parents + users tables
 *   • Profile picture upload (JPG/JPEG/PNG/WEBP, max 2 MB)
 *   • Secure password update with password_hash()
 *   • Redirect to parent/profile.php on success
 * ---------------------------------------------------------------------------
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($formErrors) && $conn !== null) {
        try {
            ...
            error_log('[Parent Edit Profile Username Check] ' . $ex->getMessage());
        }
    }
    ...
    if (empty($formErrors) && $conn !== null) {
        try {
            ...
            $chk->execute();
            if ($chk->get_result()->num_rows > 0) {
                $formErrors[] = 'This email is already registered to another parent.';
            }
            $chk->close();
        } catch (Throwable $ex) {
            error_log('[Parent Edit Profile Email Check] ' . $ex->getMessage());
        }
    }
    ...
    if (empty($formErrors) && $conn !== null) {
        ...
        try {
            ...
            if ($newProfilePictureRelative !== null) {
                ...
                if (!$stmt) {
                    throw new Exception('Prepare failed (parents update with image): ' . $conn->error);
                }
                ...
                $stmt = $conn->prepare($sqlParent);
                if (!$stmt) {
                    throw new Exception('Prepare failed (parents update): ' . $conn->error);
                }
            ...
            if (!$stmt->execute()) {
                throw new Exception('Failed to update parent profile: ' . $stmt->error);
            }
            $stmt->close();
