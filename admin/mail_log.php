 ...
 *
 * Features:
 *   • Lists all mail delivery logs from mail_logs table
 *   • Live search by MailId or Subject
 *   • Server-side pagination (Previous / Next)
 *   • Colored status badges: SUCCESS (green) / FAILED (red)
...
if ($conn instanceof mysqli) {
    try {
        ...
        // Data query — sorted DESC by sent_date
...
<body class="min-h-screen bg-slate-50 text-slate-800 antialiased flex flex-col">
      ...
      <main class="flex-1 px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-6 animate-fade-in-up">
          ...
          <nav class="flex items-center space-x-2 text-sm text-slate-500">
            ...
            <span class="text-[#E5097F] font-semibold">Mail Log</span>
          </nav>
        </div>

        <!-- TABLE CONTROLS -->
        <div class="max-w-6xl mx-auto mb-5 animate-fade-in-up" style="animation-delay: 60ms;">
          <form method="GET" action="mail_log.php" id="filterForm" class="bg-white rounded-xl shadow-sm border border-slate-200/70 px-5 py-4">
                  ...
                  <?php else: ?>
                    <?php foreach ($rows as $index => $r): ?>
                      ...
                      <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-sm text-slate-800 break-all"><?= e($mailId) ?></td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                          <button type="button"
                                  ...
                                  class="resend-btn inline-flex items-center justify-center w-9 h-9 rounded-full bg-purple-50 hover:bg-[#4A154B]
  ...
  <script>
    ...
    // ---- Live search + debounced server-side search ----
    (function () {
    ...
    // ---- Resend button (UI only — hook up your mailer here) ----
    (function () {
      document.querySelectorAll('.resend-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          if (confirmed) {
            // TODO: wire this to your mailer endpoint
            // e.g. window.location.href = 'resend_mail.php?id=' + logId;
